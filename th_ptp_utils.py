...
def text2image_ldm_stable(
    ...
    register_attention_control(model, controller)
    '''
    retister_attention_control은 모델의 구조를 수정하는 거야. 
    근데 모델의 구조를 수정하는 부분에서 AttentionControlEdit이 호출되는거야. 결국 모델의 구조를 바꿀 때 AttentionReplace도 호출되는거야. 
    근데 AttentioncontrolEdit에서 attn을 입력받아. 
    그렇다는 얘기는 모델이 이미 두 모델의 attention을 갖고 있어야 한다는 거야.......
    '''
    ...
    latent, latents = init_latent(latent, model, height, width, generator, batch_size)
    '''
    하지만 여기 init_latent에서 결국 expend를 하는 거야..!!!!
    그렇다는 얘기는 여기 init_latent에서 모델을 줄 때 
    diffusion step을 한 번 더 수행하는 것이 아니라, 기존에 우리가 기존에 돌린 latent를 가져온다는 말이 돼!  
    그렇다는 예기는 latent를 expend하는 것이 아니라, latent를 서로 다른 두 모델에서 얻은 latent를 concat할 수 있다면 이야기가 해결되는 것 같아. 
    그렇다면 앞의 가설을 시험하는 실험으로 x_t를 입력하지 않고 만약 다른 출력이 나홍다면 
...
def th_text2image_ldm_stable(
    ...
    print("th_is_running")
    # 모델에 어텐션 컨트롤러를 등록합니다.
    register_attention_control(model, controller, attn_base_=None)
    '''
    retister_attention_control은 모델의 구조를 수정하는 거야. 
    근데 모델의 구조를 수정하는 부분에서 AttentionControlEdit이 호출되는거야. 결국 모델의 구조를 바꿀 때 AttentionReplace도 호출되는거야. 
    근데 AttentioncontrolEdit에서 attn을 입력받아. 
    그렇다는 얘기는 모델이 이미 두 모델의 attention을 갖고 있어야 한다는 거야.......
    '''
    print("register_attention_control is done!!!")
    # 이미지의 높이와 너비를 설정합니다.
    height = width = 512
    # 배치 사이즈를 설정합니다.
    batch_size = len(prompt)
    ...
    return image, latent, ATTN_BASE


# 모델에 어텐션 컨트롤을 등록하는 함수입니다.
def register_attention_control(model, controller, attn_base_=None):
    ...
    def ca_forward(self, place_in_unet):
        ...
        def forward(hidden_states, encoder_hidden_states=None, attention_mask=None, temb=None,):
            ...
            attention_probs = controller(attention_probs, is_cross, place_in_unet, attn_base_)
            print("in register attention control type(attention_probs)")
            print(type(attention_probs))
            print(attention_probs.shape)
            # 이 때 __call__ method 실행됨! & output으로 attn을 출력함!
            ATTN_BASE = attention_probs
    ...
    def register_recr(net_, count, place_in_unet):
        if net_.__class__.__name__ == 'Attention':
            ...
            #ca_forward가 ca_foward.forward를 output하게 되어있고, 
                #ca_forward.forward()는 hidden_states를 output하게 되어있는 구조..!!! 
